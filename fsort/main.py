...
def main():
    ...
    parser.add_argument('--config', '--pipeline', help='Path to Python configuration file or name of Python module', required=True)
    ...
    title = f"FSORT v{__version__}: {options.config}"
    LOG.info("=" * len(title))
    LOG.info(title)
    LOG.info("=" * len(title))
